...
就是在**一个类、接口或者方法的内部创建另一个类**。

- 类：成员内部类
- 方法：局部内部类
- static：静态内部类
- 匿名内部类：就是没有名字的内部类访问外部类的局部变量时，局部变量必须用final修饰符修饰、不可变

每个内部类都能独立地继承自一个（接口的）实现，一个内部类对象可以访问创建它的外部类对象的成员，包括私有成员。**相当于变相解决的多重继承问题**。
...
- 接口中**声明的变量默认都是 final 的。成员函数默认是 public 的**。
...
- transient：将不需要序列化
- native：**因为JAVA无法对操作系统底层进行操作**，所以需要其他语言进行底层操作，一个native方法是指该方法的实现由**非Java语言实现**，比如用C或C++实现。

### Java中**static静态方法**和**普通方法**的区别

- 在类中使用**static修饰的静态方法**会随着类的定义而被分配和装载入内存中；
  - **静态不能引用非静态这一特性**，是由于静态的会随着类的定义而被分配和装载入内存中这一关键点决定的
- 而**非静态方法**属于对象的具体实例，只有在类的对象创建时在对象的内存中才有这个方法的代码段。

### 继承和组合的区别在哪？

继承目的是复用，增加新功能的能力

组合是关联关系的一种特例，体现的是整体与部分、拥有的关系，即 `has-a` 的关系。（对象里存对象）
