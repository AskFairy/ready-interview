...
- 插入时候会创建新的节点元素newNode，new Node<>(null, e, f)，
- 紧接着把新的头元素赋值给first。
- 之后判断f头节点是否存在，不存在则把头插节点也设置为尾结点。存在则用f节点的上链条的prev指针链接。
- 最后记录`size大小`、和`元素数量modCount`。modCount用在遍历时做校验，modCount != expectedModCount
...
ArrayList 是实现了基于数组的动态扩容，遍历、查询操作效率会比较高。

LinkedList 基于链表的数据结构，地址是任意的，查询性能比较低。
