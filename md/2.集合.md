...
- 如果`不等`，`查找插入位置的元素`：循环一步步走到链表的index位置，将节点返回。
  ...
  总结：`ArrayList` 中间插入，`定位`时间复杂度是`O(1)`，耗时在于**数据迁移**和容量不足的时候**扩容**。
...
- 通过定位，获取待删除元素的节点。
  - 注意：删除指定元素时的定位，需要判断元素是否为空。用不同的方式进行元素比较 `==` 或者 `equals()`
...
### （待整理）ArrayList 与 LinkedList 区别？

ArrayList 是实现了基于数组的动态扩容机制，元素地址是连续的。所以元素遍历、定位元素的效率会比较高。

LinkedList 基于链表的数据结构，元素地址是任意的，查询性能比较低。



LinkedList 适合在集合的首位有大量的插入、删除以及获取操作，提供了响应的操作方法。

头插：**LinkedList不会像ArrayList那样对元素做位移拷贝工作，所以耗时小**。

尾插：LinkedList的耗时主要消耗在`创建节点对象`的时候，ArrayList主要是消耗在`数组扩容`时的元素拷贝。也就是说，大数据尾插时，如果实现**指定ArrayList足够的容积**，*ArrayList尾插的效果更好一些*

中间插入：如果指定ArrayList的容积，防止扩容耗时。中间插入是`ArrayList` 是比`LinkedList` 数据快的。
...
但插入指定位置和删除数据时，需要移动元素，销量比较低。适合对数据进行对随机访问，和元素只添加到尾部的场景。

LinkedList 基于链表的数据结构，地址是任意的，查询性能比较低。对于新增和删除操作 add 和 remove ，LinedList 比较占优势。适用于要头尾操作或插入指定位置的场景，并且需要对数据进行多次增加删除修改的场景。
