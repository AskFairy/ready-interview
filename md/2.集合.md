...
1. **锁粒度太粗**：`Stack` 栈是在JDK1.0时代时，基于继承`Vector`，实现的。本身`Vector`就是一个不推荐使用的类，主要在于它的一些操作方法锁🔒(*synchronized*)的力度太粗，都是放到方法上。
2. **基于数组实现：**`Stack` 栈底层是使用`Vector`数组实现，在进行Push()操作时，数组结构需要通过`System.arraycopy`，进行`扩容操作`。而本身栈的特点是首尾元素的操作，也不需要遍历，使用数组结构其实并不太理想。
3. 同时在这个方法的注释上也明确标出来，推荐使用`Deque stack = new ArrayDeque();`，虽然这也是数组结构，但是它没有粗粒度的锁，同时可以申请指定空间并且在扩容时操作时也要优于`Stack` 。并且它还是一个双端队列，使用起来更灵活。
