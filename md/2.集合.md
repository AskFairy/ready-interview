...
如果列表的大小已经指定，大部分情况下是存储和遍历它们，并且遍历基本数据类型，**Array少了自动装箱**。效率快点。如果存多维数组，用Array会更方便一点。
...
**Vector 类**中的方法很多有 `synchronized` 进行修饰，虽然效率低点，是**线程安全**的。而 ArrayList 不是线程安全的，**Vector 可以设置增长因子**，而 ArrayList 不可以。但**Vector 已经被抛弃**了，如果想用线程安全的数组，可以使用 `CopyOnWriteArrayList` 。
