...
头插（addFirst()）：**LinkedList不会像ArrayList那样对元素做位移拷贝工作，所以耗时小**。

- LinkedList会一直记录头结点：first，和尾节点last，方便头尾插入。
- 插入时候会先获取first节点，并创建新的节点元素newNode，new Node<>(null, e, f)，
- 紧接着把新的头元素赋值给first。
- 之后判断f头节点是否存在，不存在则把头插节点也设置为尾结点。存在则用f节点的上链条的prev指针链接。
- 最后记录`size大小`、和元素数量modCount。modCount用在遍历时做校验，modCount != expectedModCount

尾插（add() / addLast()）：LinkedList的耗时主要消耗在`创建节点对象`的时候，ArrayList主要是消耗在`数组扩容`时的元素拷贝。也就是说，大数据尾插时，如果实现**指定ArrayList足够的容积**，*ArrayList尾插的效果更好一些*

- 与头插代码相比几乎没有什么区别，只是对first操作换成last。

中间插：

- 与ArrayList的插入一样，同样有插入区间检查。
- 如果指定位置，和记录的元素相等，直接进行尾插
- 不过不等，新用
...
ArrayList 是实现了基于数组的动态扩容，遍历、查询操作效率会比较高。但插入指定位置和删除数据时，需要移动元素，销量比较低。适合对数据进行对随机访问，和元素只添加到尾部的场景。
